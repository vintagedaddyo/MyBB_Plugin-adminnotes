...
if ($mybb->input['action'] == "version_check") {
    ...
    $table->construct_cell($mybb->version, array(
        'width' => '25%'
    ));
    $table->construct_cell("<strong>{$lang->threads}</strong>", array(
        'width' => '25%'
    ));
    $table->construct_cell("<strong>{$threads}</strong> {$lang->threads}<br /><strong>{$newthreads}</strong> {$lang->new_today}<br /><a href=\"index.php?module=forum-moderation_queue&amp;type=threads\"><strong>{$unapproved_threads}</strong> {$lang->unapproved}</a>", array(
        'width' => '25%'
    ));
    $table->construct_row();
    ...
    $table->construct_cell(PHP_VERSION, array(
        'width' => '25%'
    ));
    $table->construct_cell("<strong>{$lang->posts}</strong>", array(
        'width' => '25%'
    ));
    if ($mybb->settings['reportmethod'] == "db") {
        $table->construct_cell("<strong>{$posts}</strong> {$lang->posts}<br /><strong>{$newposts}</strong> {$lang->new_today}<br /><a href=\"index.php?module=forum-moderation_queue&amp;type=posts\"><strong>{$unapproved_posts}</strong> {$lang->unapproved}</a><br /><strong>{$reported_posts}</strong> {$lang->reported_posts}<br /><strong>{$new_reported_posts}</strong> {$lang->unread_reports}", array(
            'width' => '25%'
        ));
    } else {
        $table->construct_cell("<strong>{$posts}</strong> {$lang->posts}<br /><strong>{$newposts}</strong> {$lang->new_today}<br /><a href=\"index.php?module=forum-moderation_queue&amp;type=posts\"><strong>{$unapproved_posts}</strong> {$lang->unapproved}</a><br /><strong>{$reported_posts}</strong> {$lang->reported_posts}", array(
            'width' => '25%'
        ));
    }
    $table->construct_row();
    ...
    $table->construct_cell($db->short_title . " " . $db->get_version(), array(
        'width' => '25%'
    ));
    $table->construct_cell("<strong>{$lang->users}</strong>", array(
        'width' => '25%'
    ));
    $table->construct_cell("<a href=\"index.php?module=user-users\"><strong>{$users}</strong> {$lang->registered_users}</a><br /><strong>{$activeusers}</strong> {$lang->active_users}<br /><strong>{$newusers}</strong> {$lang->registrations_today}<br /><a href=\"index.php?module=user-awaiting_activation\"><strong>{$awaitingusers}</strong> {$lang->awaiting_activation}</a>", array(
        'width' => '25%'
    ));
    $table->construct_row();
    ...
    $table->construct_cell($serverload, array(
        'width' => '25%'
    ));
    $table->construct_cell("<strong>{$lang->attachments}</strong>", array(
        'width' => '25%'
    ));
    $table->construct_cell("<strong>{$approved_attachs}</strong> {$lang->attachments}<br /><a href=\"index.php?module=forum-moderation_queue&amp;type=attachments\"><strong>{$unapproved_attachs}</strong> {$lang->unapproved}</a><br /><strong>{$attachs['spaceused']}</strong> {$lang->used}", array(
        'width' => '25%'
    ));
    $table->construct_row();
    ...
    <div class="float_right" style="width: 48%;">';
        ...
        $table->construct_cell($form->generate_text_area("adminnotes", $adminmessage['adminmessage'], array(
    ...
    <div class="float_left" style="width: 48%;">';
        ...
        $query    = $db->simple_select('adminnotes', 'COUNT(nid) AS notes', '', array(
            'limit' => 1
        ));
        ...
        $table->construct_header("New Notes", array(
            "width" => "85%"
        ));
        $query = $db->query('SELECT * FROM ' . TABLE_PREFIX . 'adminnotes ORDER BY nid DESC LIMIT ' . $start . ', ' . $perpage);
        while ($note = $db->fetch_array($query)) {
            $userquery = $db->simple_select("users", "*", "uid=" . $note['uid']);
            $user      = $db->fetch_array($userquery);
            $table->construct_cell(format_name($user['username'], $user['usergroup'], $user['displaygroup']), array(
                "width" => "15%"
            ));
            $table->construct_cell(htmlspecialchars_uni($note['text']), array(
                "width" => "85%"
            ));
            $table->construct_row();
        }
        echo "<br /><div class=\"float_right\" style=\"width: 48%;\">";
        $table->output($lang->admin_notes);
        $table->construct_header($lang->admin_notes_public);
        $form = new Form("index.php", "post");
        $table->construct_cell($form->generate_text_area("adminnotes", $adminmessage['permissions'], array(
            'style' => 'width: 99%; height: 200px;'
        )));
        $table->construct_row();
