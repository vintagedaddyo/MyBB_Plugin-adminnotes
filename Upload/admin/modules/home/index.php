...
//		$page->output_footer();
	...
	else
	{
		...
		$query = $db->simple_select('adminnotes', 'COUNT(nid) AS notes', '', array('limit' => 1));
		...
		$table = new Table;
	    $table->construct_header("User", array("width" => "15%"));
	    $table->construct_header("New Notes", array("width" => "85%"));
		...
		while($note = $db->fetch_array($query))
	    {
			...
			$table->construct_cell(format_name($user['username'], $user['usergroup'], $user['displaygroup']), array("width" => "15%"));
			$table->construct_cell(htmlspecialchars_uni($note['text']), array("width" => "85%"));
			$table->construct_row();
	    }
		...
		$table->construct_header($lang->admin_notes_public);
		$form = new Form("index.php", "post");
		$table->construct_cell($form->generate_text_area("adminnotes", $adminmessage['permissions'], array('style' => 'width: 99%; height: 200px;')));
		$table->construct_row();
